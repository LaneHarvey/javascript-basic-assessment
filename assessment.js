...
function getValueOfProperty(x,value) {
	if (prop in x.value) {
		return x.value[prop];
	}
}
...
// to have a property called lovesCode with a value of 10.  If their name is Brack set lovesCode to 0.
// otherwise set lovesCode to 5.


function coderTest(person) {
	if (person === "Jeremy") {
		return person.lovesCode = 10;
	}
	else if (person === "Brack") {
		return person.lovesCode = 0;
	}
	else {
		return person.lovesCode = 5;
	}
}
// #15 Create a function called outside that takes in a temperature (number), a humidity(number),
// and a cloudiness(number), in that order.
